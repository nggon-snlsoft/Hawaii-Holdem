...
export class HoldemRoom extends Room<RoomState> {
	...
	cardDispensing() {
		...
		let handsMsg = this.tabelName + ' [' + handsCount.toString() + ']/';
		for ( let i = 0 ; i < this.communityCardString.length ; i++ ) {
			handsMsg += this.communityCardString[i];
			if ( i < this.communityCardString.length - 1 ) {
				handsMsg+= ', '
			}
		}
