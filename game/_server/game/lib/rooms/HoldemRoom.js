...
class HoldemRoom extends colyseus_1.Room {
    ...
    // finishProc( skip: boolean ) {
    // 	let pot = this.potCalc.GetPots(true);
    // 	pot.forEach(element => {
    // 		logger.error("Pot : " + element.total + "  players : " + element.players + "   winners : " + element.winner );
    // 	});
    //
    // 	let winAmount = 0; // for log
    // 	let allinData = [];
    //
    // 	for( let i = 0; i < this.state.entities.length; i++ ) {
    // 		if( 1 === this.state.entities[ i ].allIn ) {
    // 			allinData.push( this.state.entities[ i ] );
    // 		}
    // 	}
    //
    // 	// all-in Calculated
    // 	//--------------------------------------------------
    // 	for( let k = 0; k < allinData.length; k++ ) { // ascending
    // 		for( let j = k + 1; j < allinData.length; j++ ) {
    // 			if( allinData[ j ].roundBet < allinData[ k ].roundBet ) {
    // 				let temp: any = allinData[ k ];
    ...
    // 	}
    //
    // 	for( let k = 0; k < allinData.length; k++ ) {
    // 		let potOwners = [];
    // 		let potAmount = 0;
    // 		let allInAmount = allinData[ k ].roundBet;
    // 		let foldValue = 0;
    // 		let livePlayer = 0;
    //
    // 		for( let j = 0; j < this.state.entities.length; j++ ) {
    // 			if( false === this.state.entities[ j ].fold &&
    // 				false === this.state.entities[ j ].wait &&
    // 				2 !== this.state.entities[ j ].allIn ) {
    //
    // 				if( this.state.entities[ j ] === allinData[ k ] ) {
    // 					this.state.entities[ j ].allIn = 2;
    // 				}
    //
    // 				// 올인 보다 큰 금액을 베팅한 플레이어 리스트.
    // 				if( this.state.entities[ j ].roundBet >= allInAmount ) {
    // 					potAmount += allInAmount;
    // 					this.state.entities[ j ].roundBet -= allInAmount;
    // 					potOwners.push( this.state.entities[ j ] );
    // 				}
    // 			}
    //
    // 			// 올인과 폴드를 제외한 사용자 수.
    // 			if( 0 === this.state.entities[ j ].allIn &&
    // 				false === this.state.entities[ j ].fold &&
    // 				false === this.state.entities[ j ].wait ) {
    // 				livePlayer += 1;
    // 			}
    // 		}
    // 		logger.info( "[ finishProc ] POT [ %s ] all-in amount %s // reward amount : %s", k, allInAmount, potAmount );
    // 		logger.error(" finishProc - Pot owner length : " + potOwners.length);
    //
    // 		// 내림차순 정렬
    // 		for( let m = 0; m < potOwners.length; m++ ) {
    // 			for( let n = m + 1; n < potOwners.length; n++ ) {
    // 				if( potOwners[ n ].eval.value > potOwners[ m ].eval.value ) {
    // 					let tmp: any = potOwners[ m ];
    // 					potOwners[ m ] = potOwners[ n ];
    // 					potOwners[ n ] = tmp;
    // 				}
    // 			}
    // 		}
    ...
    // 		let potWinners = [];
    //
    // 		potWinners.push( potOwners[ 0 ] );
    //
    // 		if( livePlayer === 0 && ( k + 1 ) === allinData.length ) {
    // 			for( let o = 0; o < this.state.entities.length; o++ ) {
    // 				if( true === this.state.entities[ o ].fold ) {
    // 					foldValue += this.state.entities[ o ].roundBet;
    // 					this.state.entities[ o ].roundBet = 0;
    // 				}
    // 			}
    // 			potAmount += foldValue;
    // 			logger.info( "[ finishProc ] POT [ %s ][ ALL FOLD ] all-in amount %s // reward amount : %s", k, allInAmount, potAmount );
    // 		}
    //
    // 		// 동률 처리.
    // 		for( let m = 1; m < potOwners.length; m++ ) {
    // 			if( potOwners[ 0 ].eval.value === potOwners[ m ].eval.value ) {
    // 				potWinners.push( potOwners[ m ] );
    // 			}
    // 		}
    // 		logger.info( "[ finishProc ] POT [ %s ] winner count : %s", k, potWinners.length );
    //
    // 		for( let m = 0; m < potWinners.length; m++ ) {
    // 			for( let j = 0; j < this.state.entities.length; j++ ) {
    // 				if( potWinners[ m ].seat === this.state.entities[ j ].seat ) {
    // 					logger.info( "[ finishProc ] POT [ %s ] winner : %s // win : %s", k, this.state.entities[ j ].seat, ( potAmount / ( potWinners.length ) ) );
    // 					this.state.entities[ j ].chips += ( potAmount / ( potWinners.length ) );
    // 					this.state.entities[ j ].winAmount += ( potAmount / ( potWinners.length ) );
    // 					winAmount += ( potAmount / ( potWinners.length ) );
    // 					// this.broadcast( "MAIN_WINNER", {
    // 					// 	seat: this.state.entities[ j ].seat,
    // 					// 	win: ( potAmount / ( potWinners.length ) ),
    // 					// 	chips: this.state.entities[ j ].chips,
    // 					// 	handName: this.state.entities[ j ].eval.handName
    // 					// } );
    // 				}
    // 			}
    // 		}
    ...
    // 			0 === this.state.entities[ i ].allIn ) {
    // 			playerCount += 1;
    // 			tempSeat = this.state.entities[ i ].seat;
    // 		}
    //
    // 		if( true === this.state.entities[ i ].fold ) {
    // 			foldBetAmount += this.state.entities[ i ].roundBet;
    // 		}
    // 	}
    //
    // 	// 모두 올인 혹은 폴드를 한 경우,
    // 	if( playerCount === 1 ) {
    // 		for( let i = 0; i < this.state.entities.length; i++ ) {
    // 			if( tempSeat === this.state.entities[ i ].seat ) {
    // 				foldBetAmount += this.state.entities[ i ].roundBet;
    // 				this.state.entities[ i ].chips += foldBetAmount;
    // 				this.state.entities[ i ].winAmount += foldBetAmount;
    // 				winAmount += foldBetAmount;
    // 				// this.broadcast( "SIDE_WINNER", {
    // 				// 	seat: this.state.entities[ i ].seat,
    // 				// 	win: foldBetAmount,
    // 				// 	chips: this.state.entities[ i ].chips,
    // 				// 	handName: this.state.entities[ i ].eval.handName
    // 				// } );
    // 			}
    // 		}
    // 	}
    ...
    // 			if( false === this.state.entities[ j ].fold &&
    // 				false === this.state.entities[ j ].wait &&
    // 				0 === this.state.entities[ j ].allIn ) {
    // 				roundFinishers.push( this.state.entities[ j ] );
    // 			}
    //
    // 			if( 0 === this.state.entities[ j ].allIn &&
    // 				false === this.state.entities[ j ].wait ) {
    // 				sidePot += this.state.entities[ j ].roundBet;
    // 			}
    // 		}
    // 		logger.info( "[ finishProc ] playerCount : %s // live player count : %s // side-pot amount : %s",
    // 			playerCount, roundFinishers.length, sidePot );
    //
    // 		// 내림차순.
    // 		for( let m = 0; m < roundFinishers.length; m++ ) {
    // 			for( let n = m + 1; n < roundFinishers.length; n++ ) {
    // 				if( roundFinishers[ n ].eval.value > roundFinishers[ m ].eval.value ) {
    // 					let tmp: any = roundFinishers[ m ];
    // 					roundFinishers[ m ] = roundFinishers[ n ];
    // 					roundFinishers[ n ] = tmp;
    // 				}
    // 			}
    // 		}
    //
    // 		let sidePotWinners = [];
    // 		sidePotWinners.push( roundFinishers[ 0 ] );
    //
    // 		// 동률 처리.
    // 		for( let m = 1; m < roundFinishers.length; m++ ) {
    // 			if( roundFinishers[ 0 ].eval.value === roundFinishers[ m ].eval.value ) {
    // 				sidePotWinners.push( roundFinishers[ m ] );
    // 			}
    // 		}
    //
    // 		for( let m = 0; m < sidePotWinners.length; m++ ) {
    // 			for( let j = 0; j < this.state.entities.length; j++ ) {
    // 				let e = this.state.entities[ j ];
    // 				if( sidePotWinners[ m ].seat === e.seat ) {
    // 					logger.info( "[ finishProc ] side-pot winner : %s // win : %s", e.seat, sidePot / ( sidePotWinners.length ) );
    //
    // 					e.chips += ( sidePot / ( sidePotWinners.length ) );
    // 					e.winAmount += ( sidePot / ( sidePotWinners.length ) );
    // 					e.winHandRank = e.eval.handName;
    // 					winAmount += ( sidePot / ( sidePotWinners.length ) );
    // 					// this.broadcast( "SIDE_WINNER", {
    // 					// 	seat: e.seat,
    // 					// 	win: ( sidePot / ( sidePotWinners.length ) ),
    // 					// 	chips: e.chips,
    // 					// 	handName: e.eval.handName
    // 					// } );
    // 				}
    // 			}
    // 		}
    ...
    //
    // 	let winners = [];
    // 	for( let w = 0; w < this.state.entities.length; w++ ) {
    // 		if( 0 !== this.state.entities[ w ].winAmount &&
    // 			this.state.entities[ w ].winAmount > this.state.entities[ w ].totalBet ) {
    // 			winners.push( {
    // 				seat: this.state.entities[ w ].seat,
    // 				name: this.state.entities[ w ].name,
    // 				chips: this.state.entities[ w ].chips,
    // 				winAmount: this.state.entities[ w ].winAmount
    // 			} );
    // 		}
    // 	}
    //
    ...
    // 		skip: skip,
    // 		winners: winners,
    // 		dpPot : this.potCalc.GetPots(true),
    // 		cards: this.communityCardIndex
    // 	} );
    //
    // 	logger.info( "[ finishProc ] winners count : %s", winners.length );
    // 	winners.forEach( element => {
    // 		logger.info( "[ finishProc ] winner.seat : %s", element.seat );
    // 		logger.info( "[ finishProc ] winner.name : %s", element.name );
    // 	} );
    //
    // 	logger.info( "############################################################" );
    // 	logger.info( "@@@@@ ROUND FINISHED @@@@@" );
    // 	logger.info( "TOTAL POT : %s", this.state.pot );
    // 	logger.info( "TOTAL win : %s", winAmount );
    // 	logger.info( "############################################################" );
    // }
    updateBetSeat(seat) {
        let locSeat = seat;
